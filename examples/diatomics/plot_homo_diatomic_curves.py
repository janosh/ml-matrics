...
for model_name in model_names:
    ...
    # if the key format is "1-1", instead of "H-H", rewrite the key to "H-H" and save the updated data
    if "1-1" in homo_nuc_diatomics:
        for key in homo_nuc_diatomics:
            elem_symbol = Element.from_Z(int(key.split("-")[0])).symbol
            homo_nuc_diatomics[f"{elem_symbol}-{elem_symbol}"] = homo_nuc_diatomics.pop(
                key
            )

        with lzma.open(lzma_path, mode="wt") as file:
            json.dump(homo_nuc_diatomics, file)
